...
import com.dsp.dsp.dto.SamagraResponseDTO;
import com.dsp.dsp.dto.SamagraTokenDto;
...
public class SamgraController {
	...
	// @Value("${api.url}")
	private String uatApiUrl = "http://164.100.196.225:8085/CommonWeb/CommonApi.svc/G3tSamagraD3tails";

	private String prodApiUrl = "http://webservices.samagra.gov.in/CommonWeb/CommonApi.svc/G3tSamagraD3tails";

	@GetMapping("/genrateToken")
	public Response genrateToken() {
		...
		return Response.response("Token Genrated", HttpStatus.OK, tokenChk, null);
	...
	public Response decryptToken(@RequestBody SamagraTokenDto dto) {
		...
		return Response.response("Token Decrypted", HttpStatus.OK, jsonObject, null);
	...
	public Response abcd(@RequestBody SamagraTokenDto dto) {
...
//            //For Samgra Production URL 
			...
			if (samagraResponseDTO.getStatus() == 1) {
				...
				Response response = decryptToken(samagraTokenDto);

				return Response.response("Data Found For Provided Samagra ID", HttpStatus.OK, response.getObject(),
						null);
			} else {
				return Response.response("Data Not Found For Provided Samagra ID", HttpStatus.NOT_FOUND,
						samagraResponseDTO.getMsg(), null);
			}
		} catch (Exception e) {
			e.printStackTrace();
			return Response.response("An error occurred", HttpStatus.INTERNAL_SERVER_ERROR, null, null);
		}
	}
}